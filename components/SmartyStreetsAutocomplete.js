...
const SmartyStreetsAutocomplete = React.createClass({
  ...
  _request(text) {
    ...
    if (text.length >= this.props.minLength) {
      ...
      request.onreadystatechange = () => {
        ...
        if (request.status === 200) {
            console.log('smarty ready');
          const responseJSON = JSON.parse(request.responseText);
          console.log('smarty json', responseJSON);
          if (typeof responseJSON.suggestions !== 'undefined') {
            if (this.isMounted()) {
              this._results = responseJSON.suggestions;
              this.setState({
                dataSource: this.state.dataSource.cloneWithRows(this.buildRowsFromResults(responseJSON.suggestions)),
              });
            }
          }
          if (typeof responseJSON.error_message !== 'undefined') {
    ...
    //   request.open('GET', 'https://maps.googleapis.com/maps/api/place/autocomplete/json?&input=' + encodeURIComponent(text) + '&' + Qs.stringify(this.props.query));
        request.open('GET', 'https://us-autocomplete.api.smartystreets.com/suggest?prefix=' + encodeURIComponent(text) + '&' + Qs.stringify({ 'auth-id': 'ef96d4fc-afba-15f9-75e1-25ba1531170d', 'auth-token': '5UiRXpbVoNm9OWVoQeU1' }));
      request.send();
    } else {
      this._results = [];
      this.setState({
  ...
  _renderDescription(rowData) {
    ...
    return rowData.text || rowData.formatted_address || rowData.name;
  },
